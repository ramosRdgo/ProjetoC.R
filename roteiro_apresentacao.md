# Roteiro de Apresentação Passo a Passo (Tempo Máximo: 8 Minutos)

**Disciplina:** Comunicação e Redes  
**Projeto Prático:** Análise da Rede Cortical de Felino (`bn-cat-mixed-species_brain_1`)  
...
| **5. Síntese e Conclusão** | Seções 7 e 8 | Tabela Comparativa, Conclusões Teóricas e Perguntas | **1.5 min** (6:30 - 8:00) |

---

## 🎤 Roteiro Fala por Fala (O que dizer durante a apresentação)
...
> *"Boa noite a todos e ao professor. O nosso grupo escolheu o formato de **Projeto Prático** para analisar uma rede complexa real utilizando a linguagem Python e a biblioteca **NetworkX**.*  
> *A rede real que escolhemos analisa o **conectoma cortical do cérebro de um gato** (`bn-cat-mixed-species_brain_1`). É um grafo não-direcionado composto por **65 nós** (que representam as áreas corticais do cérebro) e **730 arestas** (que representam os feixes de conexões neurais entre essas áreas). Carregamos essa rede a partir de uma lista de arestas no formato `.edges`."*
...
> **Fala do apresentador:**  
> *"Na **análise básica**, extraímos as três métricas solicitadas no trabalho:*  
> *- A **Ordem ($N$)**: 65 nós.*  
...
>  
> *Para entender se esses números são especiais ou fruto do acaso, geramos uma **Rede Aleatória pelo modelo de Erdős-Rényi** com **exatamente a mesma ordem (65)** e **mesmo tamanho (730 arestas)**.  
> Notamos que a densidade é exatamente igual (0.3510), mas o agrupamento na rede aleatória é menor (0.3470). Isso já nos dá a primeira pista de que o cérebro possui uma organização modular estruturada, não aleatória."*
...
> **Fala do apresentador:**  
> *"Como nossa **primeira análise adicional**, investigamos o **Modelo de Pequeno Mundo (Watts-Strogatz)**.*  
...
>  
> *Geramos uma rede Watts-Strogatz com a mesma ordem $N=65$, grau médio equivalente $k=22$ e probabilidade de reconexão $p=0.1$. Notamos que o agrupamento subiu significativamente para **0.5762**, demonstrando como pequenas reconexões aleatórias em uma grade regular criam atalhos eficientes sem destruir os aglomerados locais."*
...
> **Fala do apresentador:**  
> *"Como **segunda análise adicional**, calculamos o **Diâmetro**, o **Grau Médio de Separação (Caminho Mínimo Médio $L$)** e plotamos o **Gráfico de Distribuição de Graus**.*  
>  
...
> *2. **Distribuição de Graus**: Ao olhar o histograma, vemos que enquanto a rede aleatória segue uma distribuição uniforme/Poisson centrada na média, a rede real possui **nós com alto grau (hubs corticais)** que atuam como centrais de roteamento de informação no cérebro.*  
...
> **Fala do apresentador:**  
> *"Para concluir nossa apresentação, sintetizamos todas as métricas nesta **tabela comparativa**:*  
>  
...
>  
> *As **3 grandes conclusões** do nosso trabalho são:*  
...
>  
> *Agradecemos a atenção de todos e ficamos à disposição para perguntas!"*

---

## ❓ Possíveis Perguntas do Professor e Como Responder

### ❓ Pergunta 1: *"Por que vocês escolheram comparar a rede real com uma rede de Erdős-Rényi com o mesmo N e M?"*
> **Resposta Modelo:**  
> *"Comparamos com o modelo Erdős-Rényi de mesmo $N$ e $M$ para termos uma **linha de base nula** (null model). Como a densidade é a mesma em ambas, qualquer diferença no Coeficiente de Agrupamento ou na Distribuição de Graus prova que a rede real possui uma estrutura interna organizada (modular/hierárquica) e não foi formada aleatoriamente."*
...
> **Resposta Modelo:**  
> *"Utilizamos a função `nx.read_edgelist('bn-cat-mixed-species_brain_1.edges', create_using=nx.Graph(), nodetype=int)`. Ela interpreta cada linha do arquivo como um par de vértices e constrói o objeto de grafo em memória de forma não-direcionada."*
